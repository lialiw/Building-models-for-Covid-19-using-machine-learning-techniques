...
def applyingTrainTestValid(perc_valid, modelNames,attrDropTest,attrDropValid,cut_perc,file,params_to_be_dropped, roundTwo):
    ...
    for k in range(2):
        ...
        for perc in perc_valid:
            for model in modelNames:
                ...
                x_train, y_train = separate_xValues_yValues('sample_train_'+ y_name +str(perc)+'_without_index.csv', y_name, attrDropTest[model],0,False)
                solv='liblinear'
                if k==0:
                    x_test, y_test = separate_xValues_yValues('sample_test_'+ y_name +str(perc)+'_without_index.csv', y_name, attrDropTest[model],0,False)
                else:
                    x_test, y_test = separate_xValues_yValues('df_cov_validation'+str(perc)+'_without_index.csv', y_name, attrDropValid[model],0,False)
...
def main(f_initial,f_final):
    ...
    #Initial
    attrDropTest= { 'icu': ['Unnamed: 0', 'entry_date', 'date_symptoms', 'date_died', 'dead','age_group','final_phase','total', 'group_first_phase', 'group_final_phase','group_total'],
    ...
    attrDropValid={ 'icu': ['entry_date', 'date_symptoms', 'date_died', 'dead','age_group','final_phase','total', 'group_first_phase', 'group_final_phase','group_total'],
                    'dead_with_icu': ['entry_date', 'date_symptoms', 'date_died', 'age_group','total', 'group_first_phase', 'group_final_phase','group_total','final_phase'],
                    'dead_without_icu': ['entry_date', 'date_symptoms', 'date_died', 'age_group', 'icu', 'total', 'group_first_phase', 'group_final_phase','group_total','final_phase']
                 }
    #'''
    ...
    applyingTrainTestValid(perc_valid, modelNames, attrDropTest,attrDropValid, cut_perc, f_initial, params_to_be_dropped, roundTwo)    
    if roundTwo:
        #print(params_to_be_dropped)
        new_attrDropTest, new_attrDropValid = new_attrDrop_testValid(modelNames, params_to_be_dropped, attrDropTest, attrDropValid)
